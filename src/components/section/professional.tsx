...
export default function ProfessionalSection() {
    ...
    return (
        <LayoutSection id="about" sectionRef={sectionRef}>
            ...
            <div className="justify-center w-full gap-2 grid grid-cols-1 md:grfullid-cols-2">
                ...
                <div className=" grid grid-cols-3 w-full gap-2" >
                    {ProfileData.experience.map((skill) => (
                        <Card className=" flex flex-row items-center justify-start gap-2 bg-black/60 border-md border-sky-700 text-left p-2 h-full text-white " key={skill.name}>
                    ...
                    <Card className="bg-black/60 border-md  text-center p-4 h-full text-white justify-center">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2">
                            {ProfileData.fontend.map((skill) => (
                                <div className="flex flex-row gap-2 rounded-lg " key={skill.name}>
                                    <img src={skill.icon} className={`h-5 w-5 rounded-lg ${skill.color}`} />
                                    <span>{skill.name}</span>
